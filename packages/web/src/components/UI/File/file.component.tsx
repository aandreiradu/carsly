...
export type FileComponentHnadlers = {
  getImages: () => File[];
};

const imageTypeRegex = /image\/(png|jpg|)/gm;

const FileComponent = forwardRef<FileComponentHnadlers, FileComponentProps>(
  ({ withDragDrop, maxAcceptedFiles, wrapperClasses, buttonClasses, withCountHeader }, ref) => {
    ...
    const handleButtonClick = useCallback(() => {
      ...
      //   console.log('add', fileInputRef.current);
    ...
    const changeHandler = (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (files && files.length > 0) {
        ...
        /*
          ** currently we'll store only the current state. We're not going to append new files
          to existing files
          */
    ...
    const handleRemoveImage = async (image: string) => {
      ...
      // if (typeof ref === 'function') {
      //   console.log('remove', fileInputRef.current);
      //   ref(fileInputRef.current);
      // }
    };
