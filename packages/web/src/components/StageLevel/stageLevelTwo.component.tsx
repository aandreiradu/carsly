...
import { SellNowStageTwoProps } from '../../pages/SellNow/types';
...
type StageLevelProps = {
  ...
  form: UseFormReturn<SellNowStageTwoProps, any>;
  onSubmit: () => void;
  isLoading?: boolean;
};
...
const StageLevelTwo = ({
  ...
  return (
    <form className="mt-3 w-full grid grid-cols-2 gap-5" onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <Select
                ...
                cachedValue={form.getValues('model')}
              />
            )}
          />
        </div>
        <span className="lg:pl-1 text-sm text-red-500">{form.formState.errors?.model?.message}</span>
      </div>
      <div className="flex flex-col">
        <div className="relative border border-black w-full py-1 px-1 bg-transparent flex flex-col flex-1 rounded-xl max-h-16">
          <Label className="text-sm text-black border-none focus:outline-none active:outline-none bg-transparent px-1">
          ...
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <Select
                ...
                cachedValue={form.getValues('fuel')}
              />
            )}
          />
        </div>
        <span className="lg:pl-1 text-sm text-red-500">{form.formState.errors.fuel?.message}</span>
      </div>
